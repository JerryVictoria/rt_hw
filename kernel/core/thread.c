...
void pok_thread_init(void)
{
   ...
   pok_threads[IDLE_THREAD].weight                     = 255;

   pok_threads[IDLE_THREAD].weight                     = 1;
...
{
   // printf("pok_partitions[partition_id].thread_index: %d\n", pok_partitions[partition_id].thread_index);
   uint32_t id;
...
#ifdef DYNAMIC_READ_INPUT_MODE
         && pok_partitions[partition_id].mode != POK_PARTITION_MODE_NORMAL
#endif
         )
   {
   ...
   // add pok_threads weight
   if(attr->weight > 0)
   {
      // 1 ~ 255
      pok_threads[id].weight        = ((attr->weight > 255) ? 255: attr->weight);
   }
...
{
   // pok_threads[current_thread].remaining_time_capacity = POK_THREAD_DEFAULT_TIME_CAPACITY;
   pok_sched();
}
